...
class FusionNode(nn.Module):
    ...
    def __init__(self,
        ...
        self.upsample_mode = upsample_cfg.copy()
        self.op_num = op_num
    ...
    def _resize(self, x, size):
        if x.shape[-2:] == size:
            ...
            return F.interpolate(x, size=size, **self.upsample_mode)
        else:
            _, _, h, w = x.size()
            x = F.max_pool2d(
...
class RCFPN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        c2, c3, c4, c5 = feats
        c6 = F.max_pool2d(c5, 1, stride=2)
        # fixed order
        p2 = self.RevFP['p2']([c2, c3], out_size=c2.shape[-2:])
        p3 = self.RevFP['p3']([c3, c4, p2], out_size=c3.shape[-2:])
        ...
        p2, p3, p4, p5, p6 = self.CSN([p2, p3, p4, p5, p6])

        return tuple([p2, p3, p4, p5, p6])